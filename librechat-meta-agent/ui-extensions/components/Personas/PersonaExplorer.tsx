...
export default function PersonaExplorer({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-stone-50 to-stone-50 text-stone-900">
      {/* Header */}
      <div className="border-b border-stone-200 bg-white/50 backdrop-blur-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-stone-100 rounded-xl border border-stone-200 focus:border-indigo-500 focus:outline-none"
            />
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-8">
          ...
          >
            <div className="sticky top-32 space-y-6">
              {/* Categories */}
              <div>
                <h3 className="text-sm font-semibold mb-3 text-stone-500">CATEGORIES</h3>
                ...
                <div className="space-y-2">
                  {featuredPersonas.slice(0, 3).map((persona) => (
                    <button
                      ...
                      className="w-full text-left p-2 hover:bg-stone-200/50 rounded-lg transition-colors"
                    >
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-stone-500 text-lg">No personas found</p>
...
function PersonaCard({ persona, isLiked, onLike, onFork, onClick, featured }: PersonaCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={clsx(
        'relative group bg-stone-100/50 rounded-xl p-5 border-2 transition-all text-left hover:border-indigo-500 hover:shadow-xl hover:shadow-indigo-500/20',
        featured
          ? 'border-yellow-500/30 bg-gradient-to-br from-yellow-900/10 to-stone-100'
          : 'border-stone-200'
      )}
    >
      {featured && (
      ...
      <div className="flex items-start gap-3 mb-3">
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-teal-500 flex items-center justify-center overflow-hidden flex-shrink-0">
          {persona.avatar_url ? (
            <img src={persona.avatar_url} alt={persona.name} className="w-full h-full object-cover" />
          ) : (
            <CategoryIcon className="w-6 h-6 text-stone-900" />
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-stone-500 capitalize">{persona.category}</p>
        </div>
      </div>
...
function PersonaPreviewModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl border border-stone-200 max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-stone-200">
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-teal-500 flex items-center justify-center overflow-hidden flex-shrink-0">
              {persona.avatar_url ? (
                <img src={persona.avatar_url} alt={persona.name} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-stone-500 capitalize">{persona.category}</p>
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-container space-y-6">
          {/* Description */}
          <div>
            <h3 className="text-sm font-semibold mb-2 text-stone-500">DESCRIPTION</h3>
            <p className="text-stone-700">{persona.description}</p>
          </div>

          {/* Capabilities */}
          <div>
            <h3 className="text-sm font-semibold mb-3 text-stone-500">CAPABILITIES</h3>
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(persona.capabilities).map(([key, enabled]) => {
                ...
                return (
                  <div
                    key={key}
                    className="flex items-center gap-2 px-3 py-2 bg-stone-100/50 rounded-lg"
                  >
          ...
          {/* Personality */}
          <div>
            <h3 className="text-sm font-semibold mb-3 text-stone-500">PERSONALITY</h3>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="bg-stone-100/50 rounded-lg p-3">
                <p className="text-xs text-stone-500 mb-1">Verbosity</p>
          ...
          {persona.starter_prompts.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold mb-3 text-stone-500">STARTER PROMPTS</h3>
              <div className="space-y-2">
                {persona.starter_prompts.map((prompt, index) => (
                  <div
                    key={index}
                    className="px-4 py-2 bg-stone-100/50 rounded-lg text-sm text-stone-700"
                  >
                    {prompt}
                  </div>
                ))}
          ...
          {/* Stats */}
          <div>
            <h3 className="text-sm font-semibold mb-3 text-stone-500">STATISTICS</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-stone-100/50 rounded-lg p-3 text-center">
